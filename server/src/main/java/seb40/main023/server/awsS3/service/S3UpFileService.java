...
public class S3UpFileService {
    ...
    public String uploadLogin(MultipartFile multipartFile) throws IOException {
...
//        s3UpFiles.setMember(memberService.findVerifiedMember(memberId)); // 멤버 세팅
//        s3UpFiles.setMember(memberRepository.findById(memberId)); // 멤버 세팅
//        s3UpFiles.getMember().setImgUrl(s3UpFiles.getUpFileUrl());
    ...
    public String uploadLuckMango(MultipartFile multipartFile) throws IOException {
        String s3FileName = UUID.randomUUID() + "-" + multipartFile.getOriginalFilename();
